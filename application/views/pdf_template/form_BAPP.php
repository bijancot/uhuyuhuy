<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>FORM BERITA ACARA PEMERIKSAAN PEKERJAAN</title>
    <style>
        html,
        body {
            font-family: Arial, Helvetica, sans-serif;
            font-size: 14px;
            width: 124vw;
            height: 175.4vw;
        }

        .w-50 {
            min-width: 50%;
            max-width: 50%;
        }

        .w-100 {
            min-width: 100%;
        }

        .table-center {
            margin-right: auto;
            margin-left: auto;
        }

        .p-min {
            padding: .5em .3em;
        }

        .pl-min {
            padding-left: .3em;
        }

        .pt-min {
            padding-top: .5em;
        }

        .pb-min {
            padding-bottom: .4em;
        }

        .valign-middle {
            vertical-align: middle;
        }

        .p-max {
            padding: 2em;
        }

        .mb-max {
            margin-bottom: 3em;
        }

        .mb-min {
            margin-bottom: 1.5em;
        }

        .mt-max {
            margin-top: 3em;
        }

        .mt-min {
            margin-top: 1.5em;
        }

        .border-collapse {
            border-collapse: collapse;
        }

        .border-1 {
            border: 1px solid black;
        }

        .border-2 {
            border: 2px solid black;
        }

        .border-right {
            border-right: 1px solid black;
        }

        .border-bottom {
            border-bottom: 1px solid black;
        }

        .border-left {
            border-left: 1px solid black;
        }

        .border-top {
            border-top: 1px solid black;
        }

        .text-title {
            font-size: 24px;
        }

        .text-regular {
            font-size: 12px;
        }

        .text-regular-sm {
            font-size: 10px;
        }

        .font-weight-regular {
            font-weight: 400;
        }

        .font-weight-bold {
            font-weight: 800;
        }

        .table-layout-fixed {
            table-layout: fixed;
        }

        .text-align-left {
            text-align: left;
        }

        .text-align-center {
            text-align: center;
        }

        .text-align-justify {
            text-align: justify;
        }

        .text-vertical {
            writing-mode: vertical-rl;
        }

        .watermark::before {
            position: absolute;
            top: 30%;
            left: 37%;
            /* content: "BAPP"; */
            z-index: 99;
            font-size: 80px;
            transform: rotate(-30deg);
            color: rgba(0, 0, 0, .15);
            text-align: center;
            line-height: 1.8em;
        }

        .wrapper-page {
            page-break-after: always;
        }

        .wrapper-page:last-child {
            page-break-after: avoid;
        }

        .bg-grey {
            background-color: #C0C0C0;
        }

        .absolute {
            position: absolute;
        }

        .pos-right {
            right: 320px;
        }

        .pos-left {
            left: 0;
        }

        .float-left {
            float: left;
        }

        .float-right {
            float: right;
        }
    </style>
</head>

<body>
    <div class="wrapper-page">
        <div class="watermark"></div>
        <table class="border-1 border-collapse w-100 valign-middle mb-min">
            <tr>
                <th class="text-regular font-family-arial text-align-center pt-min border-1" colspan="2">
                    <img src="data:image/png;base64," height="25px" width="25px">
                    &nbsp;<strong class="valign-middle">UNITED TRACTORS</strong>
                </th>
                <th class="text-align-center border-1" colspan="2">
                    <strong>
                        FORM BERITA ACARA PEMERIKSAAN PEKERJAAN
                    </strong>
                </th>
                <th class="text-align-center border-1" colspan="3"><strong>INTEGRASI SISTEM</strong></th>
            </tr>
            <tr>
                <td class="text-regular text-align-center border-right" colspan="2">Jl. Raya Bekasi KM 22 Cakung <br> Jakarta Timur</td>

                <td class="text-regular" style="width: 9vw">
                    <div class="text-regular pl-min pt-min">
                        <div><strong>No. Dokumen</strong></div>
                        <div><strong>Revisi</strong></div>
                        <div><strong>Hal</strong></div>
                    </div>
                </td>

                <td class="text-regular">
                    <div class="text-regular pl-min pt-min">
                        <div> : FORM 007/PROS-MFP-MLK3-015</div>
                        <div> : 1</div>
                        <div> : 1 Dari 1</div>
                    </div>
                </td>

                <td class="text-regular text-align-center border-left" colspan="3">
                    <strong>
                        ISO 9001 : 2015 ; 1SO <br>
                        14001 : 2015 ; OHSAS <br>
                        18001 : 2018 &amp; SMK3
                    </strong>
                </td>
            </tr>
        </table>

        <table class="w-100 mb-min">
            <tr>
                <th class="text-align-center">
                    <strong>
                        <u>BERITA ACARA PEMERIKSAAN PEKERJAAN</u>
                    </strong>
                </th>
            </tr>
            <tr>
                <td class="text-regular">
                    <div class="text-align-center pl-min pt-min">
                        <strong><u>No. <?= $docNo?></u></strong>
                    </div>
                </td>
            </tr>
        </table>
        <br>
        <table>
            <tr>
                <td class="text-regular" style="float: left">
                    Pada hari ini <?= $dateDay?> (<?= $dateDayLatin?>) <?= $dateMonth?> (<?= $dateMonthLatin?>), kami yang bertanda tangan dibawah ini :
                </td>
            </tr>
        </table>
        <br>
        <dd>
            <table>
                <tr>
                    <td class="text-regular" style="float: left">
                        1. 
                    </td>
                    <td class="text-regular" style="float: left">
                        Nama
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        <?= $nama?>
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Jabatan
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        <?= $jabatan?>
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Nama Badan Usaha
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        <?= $badanUsaha?>
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Alamat
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        <?= $alamat?>
                    </td>
                </tr>
            </table>
        </dd>
        <br><br>
        <dd>
            <table>
                <tr>
                    <td class="text-regular" style="float: left">
                        2. 
                    </td>
                    <td class="text-regular" style="float: left">
                        Nama
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        Bagus Setiawan
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Jabatan
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        GA Department Head
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Nama Badan Usaha
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        PT United Tractors Tbk
                    </td>
                </tr>
                <tr>
                    <td class="text-regular" style="float: left">
                    </td>
                    <td class="text-regular" style="float: left">
                        Alamat
                    </td>
                    <td class="text-regular" style="float: center">
                        :
                    </td>
                    <td class="text-regular" style="float: left">
                        Jalan Raya Bekasi KM 22 Cakung, Jakarta Timur
                    </td>
                </tr>
            </table>
        </dd>
        <br><br>
        <table class="w-100 mb-min text-align-left">
            <tr>
                <td class="text-regular">
                    Setelah melaksanakan Pemeriksaan Bersama Pekerjaan Pembangunan <?= $project[0]->project_name?>
            </tr>
            <tr>
                <td class="text-regular">
                    Selanjutnya menyetujui dan menyepakati pekerjaan yang telah dilakukan Kontraktor <?= $mulaiPengerjaan?> sampai dengan tanggal <?= $akhirPengerjaan?> telah mencapai Progress sebesar <?= $progress?>% .
                </td>
            </tr>
            <br><br>
            <tr>
                <td class="text-regular">
                    Demikian Berita Acara Pemeriksaan Pekerjaan ini dibuat rangkap 2 ( dua ) dengan bunyi dan kekuatan hukum yang sama.
                </td>
            </tr>
            <tr>
                <td class="text-regular">
                    <br>
                    <?= $area?> , <?= $date?>
                </td>
            </tr>
        </table>
        <br>
        <table class="border-collapse w-100 valign-middle mb-min">
            <tr>
                <th class="text-regular font-family-arial text-align-center pt-min" colspan="2">
                    <div>Dibuat,</div>
                    <div>Kontraktor</div>
                </th>
                <th class="text-align-center" colspan="2">
                    <div>Diperiksa,</div>
                    <div>Konsultan</div>
                </th>
                <th class="text-align-center" colspan="2">
                    <div>Diketahui,</div>
                    <div>PT United Tractors Tbk</div>
                </th>
            </tr>

            <!-- PATH TTD -->
            <tr>
                <td class="text-regular text-align-center" colspan="2">
                    <!-- <img src="https://ut-staging.bgskr-project.my.id/images/ttd/3015b0a71809a822fc9a64790c2d5a5b.png" alt="TTD" width="150"> -->
                    <br><br><br><br><br>
                </td>
                <td class="text-regular text-align-center" colspan="2">
                </td>
                <td class="text-regular text-align-center" colspan="2">
                </td>
            </tr>
            <!-- PATH TTD -->

            <tr>
                <td class="text-regular text-align-center" colspan="2">
                    <div class="text-align-center">
                        <u>.......................................</u>
                    </div>
                    <div class="text-align-center">
                        .................................
                    </div>
                </td>

                <td class="text-regular text-align-center" colspan="2">
                    <div class="text-align-center">
                        <u>.......................................</u>
                    </div>
                    <div class="text-align-center">
                        ** .................................
                    </div>
                </td>

                <td class="text-regular text-align-center" colspan="2">
                    <div class="text-align-center">
                        <u><?= $picName?></u>
                    </div>
                    <div class="text-align-center">
                        * Branch / Site Operation Head
                    </div>
                </td>
            </tr>
        </table>

        <table class="border-collapse w-100 valign-middle mb-min">
            <tr>
                <th class="text-regular font-family-arial text-align-center pt-min" colspan="2">
                    <div>Diperiksa,</div>
                    <div>PT United Tractors Tbk</div>
                </th>
                <th class="text-align-center" colspan="2">
                    <div>Disetujui,</div>
                    <div>PT United Tractors Tbk</div>
                </th>
                <th class="text-align-center" colspan="2">
                </th>
            </tr>

            <!-- PATH TTD -->
            <tr>
                <td class="text-regular text-align-center" colspan="2">
                    <!-- <img src="https://ut-staging.bgskr-project.my.id/images/ttd/3015b0a71809a822fc9a64790c2d5a5b.png" alt="TTD" width="150"> -->
                    <br><br><br><br>
                </td>
                <td class="text-regular text-align-center" colspan="2">
                </td>
                <td class="text-regular text-align-center" colspan="2">
                </td>
            </tr>
            <!-- END PATH TTD -->
            
            <tr>
                <td class="text-regular text-align-center" colspan="2">
                    <div class="text-align-center">
                        <u>Maya Alfianti S</u>
                    </div>
                    <div class="text-align-center">
                        Project Management Section Head
                    </div>
                </td>

                <td class="text-regular text-align-center" colspan="2">
                    <div class="text-align-center">
                        <u>Bagus Setiawan</u>
                    </div>
                    <div class="text-align-center">
                        GA Dept. Head
                    </div>
                </td>

                <td class="text-regular text-align-center" colspan="2">
                    
                </td>
            </tr>
        </table>
        <div class="text-regular">Catatan :</div>
        <div class="text-regular">* Jika project dikerjakan di area Cabang atau Site</div>
        <div class="text-regular">** Jika system pengawasan project menggunakan jasa konsultan</div>
    </div>
</body>

</html>