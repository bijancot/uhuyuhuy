...
class ProjectType extends RestController {
    ...
    public function index_get(){
        try{
            ...
            $type = $DB2->order_by('type_no ASC')->get('master_project_type')->result();
    ...
    public function index_post(){
        try{
            ...
            $type = $DB2->get_where('master_project_type', ['type_name' => $param['typeName']])->result();
            if($type != null) $this->response(['status' => false, 'message' => 'Nama type telah digunakan!'], 200);
             // End Validation
            
            $formData['type_name'] = $param['typeName'];
            $DB2->insert('master_project_type', $formData);
    ...
    public function changeStatus_put(){
        try{
            ...
            // Validation
            if(empty($param['idType'])) $this->response(['status' => false, 'message' => 'Parameter tidak cocok' ], 200);
            $user = $DB2->get_where('master_user', ['user_no' => $jwt->user_no])->row();
            if($jwt->user_sess != $user->user_session || $user->user_session == null) $this->response(['status' => false, 'message' => 'Login terlebih dahulu!'], 401);
            if($user->user_role != "Admin PICP") $this->response(['status' => false, 'message' => 'Anda tidak memiliki hak akses!'], 200);

            $tpe = $DB2->get_where('master_project_type', ['type_no' => $param['idType']])->result();
            if($tpe == null) $this->response(['status' => false, 'message' => 'type tidak terdaftar!'], 200);

            if($tpe[0]->type_status == 0){
                $param['status'] = 1;
            }else{
                $param['status'] = 0;
            };
            ...
            $DB2->where('type_no', $param['idType'])->update('master_project_type', ['type_status' => $param['status']]);
            $this->response(['status' => true, 'message' => 'Berhasil mengubah status type!'], 200);
        }catch(Exception $exp){
            $this->response(['status' => false, 'message' => 'Login terlebih dahulu!'], 401);
        }
    }
    ...
    public function detail_get($id){
        try{
            ...
            $type = $DB2->get_where('master_project_type', ['type_no' => $id])->row();
            if(!empty($type)){
                $this->response(['status' => true, 'message' => 'Data berhasil ditemukan', 'data' => $type], 200);
            }else{
                $this->response(['status' => true, 'message' => 'Data tidak ditemukan'], 200);
    ...
    public function index_put(){
        try{
            ...
            // Validation
            if(empty($param['idType']) || empty($param['typeName'])) $this->response(['status' => false, 'message' => 'Parameter tidak cocok' ], 200);
            $user = $DB2->get_where('master_user', ['user_no' => $jwt->user_no])->row();
            if($jwt->user_sess != $user->user_session || $user->user_session == null) $this->response(['status' => false, 'message' => 'Login terlebih dahulu!'], 401);
            if($user->user_role != "Admin PICP") $this->response(['status' => false, 'message' => 'Anda tidak memiliki hak akses!'], 200);
            // End Validation

            $formData['type_no']       = $param['idType'];
            $formData['type_name']     = $param['typeName'];

            $DB2->where('type_no', $param['idType'])->update('master_project_type', $formData);
