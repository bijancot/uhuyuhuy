...
class FormBAST extends RestController
{
    ...
    public function generate()
    {
        ...
        $file_pdf = $namaUser . "_BAST" . '_' . time();
        $path_pdf = 'uploads/project/bast/' . $file_pdf . '.pdf';

        $paper = 'A4';
        $orientation = 'portrait';

        $html = $this->load->view("pdf_template/form_BAST", '', true);

        $resPdf = $this->pdfgenerator->generate($html, $file_pdf, $paper, $orientation);
        if (!is_dir('./uploads/project/bast/')) {
            mkdir('./uploads/project/bast/', 0777, TRUE);
        }
