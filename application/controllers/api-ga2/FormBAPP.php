...
class FormBAPP extends RestController
{
    ...
    public function generate()
    {
        ...
        $file_pdf = $namaUser . "_BAPP" . '_' . time();
        $path_pdf = 'uploads/project/bapp/' . $file_pdf . '.pdf';

        $paper = 'A4';
        $orientation = 'portrait';

        $html = $this->load->view("pdf_template/form_BAPP", '', true);

        $resPdf = $this->pdfgenerator->generate($html, $file_pdf, $paper, $orientation);
        if (!is_dir('./uploads/project/bapp/')) {
            mkdir('./uploads/project/bapp/', 0777, TRUE);
        }
